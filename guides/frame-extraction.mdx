...
description: "Understand how claude-real-video selects frames using ffmpeg scene-change detection combined with a configurable density floor to cover both fast cuts and static screencasts."
---
...
- **`not(mod(n,{every_n}))`** — fires on every *N*th frame number, providing the density floor. `every_n` is computed as `round(fps × fps_floor)`, so at 25 fps with `--fps-floor 1.0` it keeps one frame per second regardless of how static the footage is.
